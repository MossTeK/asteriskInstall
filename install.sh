...
#add repositorys to apt
add-apt-repository -y ppa:ondrej/php 
add-apt-repository -y universe 
apt update -y 

#check if the packages are installed and if not install them
packages=("libedit-dev" "nodejs" "npm" "libapache2-mod-php7.4" "php7.4" "php7.4-mysql" "php7.4-cli" "php7.4-common" "php7.4-imap" "php7.4-ldap" "php7.4-xml" "php7.4-fpm" "php7.4-curl" "php7.4-mbstring" "php7.4-zip" "php7.4-gd" "php7.4-gettext" "php7.4-xml" "php7.4-json" "php7.4-snmp" "lsb-release" "ca-certificates" "apt-transport-https" "software-properties-common" "gnupg2" "git" "curl" "wget" "libnewt-dev" "libssl-dev" "libncurses5-dev" "subversion" "libsqlite3-dev" "build-essential" "libjansson-dev" "libxml2-dev" "uuid-dev" "mariadb-server" "apache2")

for package in "${packages[@]}"; do
    if ! dpkg -l | grep -q "^ii  $package "; then
        apt install -y "$package"
    else
        echo "$package is already installed." | tee install.log
    fi
done
apt upgrade -y 
...
# run everything in $commands, if any fail print out error message
build_asterisk() {
    for cmd in "$@"; do
        $cmd
        local status=$?
        if [ $status -ne 0 ]; then
            echo "Error: Command '$cmd' failed with exit code $status." | tee ./install.log
            exit $status
        fi
    done
}
...
#open ports for ssh http https RTP and SIP
